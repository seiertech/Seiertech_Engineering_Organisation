...
RELOCATED into ems_engine.scan.scanner as part of the Python EMS Engine
package build (see PYTHON_ENGINE_BUILD_PLAN.md, Step 3). Logic is unchanged
from the validated version in .github/scripts/scan_repo.py — this adds an
importable `scan_repository()` function for use by ems_engine.personas
while preserving the original CLI entry point so the existing GitHub
Actions workflow keeps working unmodified during the transition.
...
def scan_repository(repo_url: str, github_token: str = "") -> dict:
    """
    Importable entry point for ems_engine consumers (e.g. the persona
    executor). Same logic as the CLI path below, returns the result dict
    directly instead of writing to a file — callers that want the file
    written should use the CLI/main() path or write the returned dict
    themselves.
    """
    with tempfile.TemporaryDirectory() as tmpdir:
        clone_path = os.path.join(tmpdir, "repo")
        ok, err = clone_shallow(repo_url, github_token, clone_path)

        if not ok:
            return {
                "scan_status": "FAILED",
                "error": err,
                "note": "No code evidence available — clone failed.",
            }
